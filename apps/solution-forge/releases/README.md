...
| --- | --- | --- | --- |
| `DynamicsProSolutionAdminConsole_1.0.0.24_managed.zip` | 1.0.0.24 | 2026-08-19 | **9** `pro_` tables incl. the three **delta** columns on `pro_transferentry` · 3 transfer-executor flows (Execute Package / Execute Cell / Scheduler), host operations on `organization: "current"` · Code App (chunk-split bundle with two **lazily loaded** workspaces, incl. the first-run **Self-Provisioning Wizard**, Role Comparer, the security concept document, the entry dialog's **Write plan**, the OData Browser's **cloud-flow copy buttons** and **record field search**, shareable workspace links, the Dual-Write cockpit's **running-version lookup** and **per-environment view**, **lookup and choice columns resolved to names** in the transfer preview and import history, plus a transfer column picker that offers **only transferable columns**, and **row-level error messages** in the transfer run log) · **executor flows rebuilt in 1.0.0.24** (Execute Cell — row-level error messages with a failed-row count) · 1 connection reference `pro_CR_SAC_Dataverse` · 1 security role `INT \| DEPLOYMENT MANAGER` · **unchanged since 1.0.0.17:** the data model |
...
explicitly (1.0.0.17 added three columns; 1.0.0.24 changes a flow with no
