...
| --- | --- | --- | --- |
| `DynamicsProSolutionAdminConsole_1.0.0.29_managed.zip` | 1.0.0.29 | 2026-09-16 | **9** `pro_` tables incl. `pro_transferrun.pro_flowrun_str` · 3 transfer-executor flows (Execute Package / Execute Cell / Scheduler), host operations on `organization: "current"` · Code App (chunk-split bundle with two **lazily loaded** workspaces; see the changelog for the feature list) · 1 connection reference `pro_CR_SAC_Dataverse` · 1 security role `INT \| DEPLOYMENT MANAGER` · **no data-model or flow change since 1.0.0.25**; merge now reads the release solution raw and reuses the plan, so it starts moving immediately · swept over the complete package with **token** patterns — no other customer's name, solution, entity, DevOps organisation or prefix |
...
1.0.0.26 through 1.0.0.29 change neither).
